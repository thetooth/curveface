...
type EtcdClient interface {
	...
	// Has returns true if key exists in directory
	Has(directory, key string) (bool, error)
...
// Has returns true if the requested key is in a directory
func (etcdClient *SimpleEtcdClient) Has(directory, key string) (bool, error) {
	api := client.NewKeysAPI(etcdClient.etcd)
	options := &client.GetOptions{Sort: true, Recursive: false}
	response, err := api.Get(context.Background(), directory, options)
